...
Since mod 154 the seeds' `test.sh` also carries the **reference shard split**: when `DOCEX_TEST_SLOTS > 1` it runs only this slot's deterministic `1/N` share of the collected `tests/` node-ids (a modulo over `--collect-only`), and is byte-identical to before when the vars are unset. It is a **reference, not a mandate** (`tests.md § Injected environment`) — the seeds are downstream projects' copy source, so they exemplify how a `pytest` shim consumes `DOCEX_TEST_SLOT`/`DOCEX_TEST_SLOTS` under `docex test --slots N` while keeping the union of the `N` shards equal to the whole suite.
