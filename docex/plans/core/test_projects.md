...
So the seeds keep the tiers conceptually distinct even though one `test.sh` runs the whole suite. DB-backed tests (`test_jobs_smoke.py` / `test_jobs_concurrency.py`) assert only **outcomes in shared state** (and count the live worker as a third claimer, identified by the "no handler" error it stamps on unrecognized job names), while agency-shaped assertions (`test_jobs_alogic.py`) run against a stub queue. That is where `hex_overview.md § Tests` puts them anyway; the `test`-env constraint simply makes the doctrine's tiering non-optional — a discipline the tests observe, not a shim boundary.

Since mod 154 the seeds' `test.sh` also carries the **reference shard split**: when `DOCEX_TEST_SLOTS > 1` it runs only this slot's deterministic `1/N` share of the collected `tests/` node-ids (a modulo over `--collect-only`), and is byte-identical to before when the vars are unset. It is a **reference, not a mandate** (`tests.md § Codebase Test Env Vars`) — the seeds are downstream projects' copy source, so they exemplify how a `pytest` shim consumes `DOCEX_TEST_SLOT`/`DOCEX_TEST_SLOTS` under `docex test --slots N` while keeping the union of the `N` shards equal to the whole suite.
