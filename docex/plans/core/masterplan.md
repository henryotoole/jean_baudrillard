...
- **Provider set = (CORE-targeted `uses` entries) ∪ (`web`-network core services)**, minus `scheduler` core services. Both arms are load-bearing. The first is the declared interface graph. The second catches every publicly reachable boundary even when nothing *inside* the project uses it — drive the set off `uses` alone and a public edge silently loses its contract, taking with it everything the health gate has to validate.
