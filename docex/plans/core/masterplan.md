...
| `docs <scaffold\|check>` | both | `project.yml`, `infra.yml` (codebases), `plans/design/**` | `scaffold`: idempotent creation of the standard design-doc set under `plans/design` (never clobbers; empty standard dirs get a `.gitkeep`); `check`: read-only — exit code + per-problem report over the **missing-standard-file** and **doc-reachability** (orphan) checks. Both consult ONE canonical standard-set definition (`src/docex/docs/standard_set.py`). `docs check` also runs as a blocking gate of `docex check`; both skip when the project has no `plans/design` yet. (`docs adr` — index generation — is a later mod.) |
...
- `check` invokes `compile` (to verify it succeeds), `build` (via `docker build` during test), and `test`. It also runs `docs check`'s two gates (missing-standard-file + doc reachability) against the worktree, both blocking; they skip when the project has no `plans/design`.
