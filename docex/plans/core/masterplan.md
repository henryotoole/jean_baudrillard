...
| `docs <scaffold\|check\|adr\|linkmap>` | both | `project.yml`, `infra.yml` (codebases), `plans/design/**`, `core/<cb>/src/**` (linkmap `code_level`) | `scaffold`: idempotent creation of the standard design-doc set under `plans/design` (never clobbers; empty standard dirs get a `.gitkeep`); `check`: read-only — exit code + per-problem report over the **missing-standard-file**, **doc-reachability** (orphan), and **ADR-index-freshness** checks. Both consult ONE canonical standard-set definition (`src/docex/docs/standard_set.py`); `check` and `adr` additionally share ONE canonical ADR-index render (`src/docex/docs/adr.py`). `docs check` also runs as a blocking gate of `docex check`; both skip when the project has no `plans/design` yet. `adr`: regenerate `adr_index.md` (all ADRs) and `adr_active.md` (accepted + not superseded) from `plans/design/adrs/` — deterministic and idempotent, each file carrying a "generated by `docex docs adr`" marker. `docs check` (and the `docex check` gate) also flag an ADR index that has drifted from its sources as a blocking problem. `linkmap <design_docs\|code_level>`: emit the doc/code link graph (nodes + edges) as deterministic sorted JSON on stdout for outside agents/skills. `check`'s reachability now **consumes** this same graph (`src/docex/docs/linkmap.py::build_linkmap`), so the orphan gate and the emitted linkmap are one source of truth. |
...
| `check` | both | feature branch + origin/main | ephemeral git worktree, runs git/version checks, the design-doc structure checks (`docs check`), contract checks, build, test — a [durable job](#durable-jobs-the-job-substrate); blocks + exits with the run's code (durable underneath), or `--detach` → a handle. On success writes a `.docex/checks/` provenance record for `merge` to trust forward |
...
- `check` invokes `compile` (to verify it succeeds), `build` (via `docker build` during test), and `test`. It also runs `docs check`'s three gates (missing-standard-file + doc reachability + ADR-index freshness) against the worktree, all blocking; they skip when the project has no `plans/design`.
...
- `.docex/checks/latest.json` — the provenance record a fully-green `check` writes (`{feature_tip, origin_main, merged_tree_sha, checked_at, docex_version}`) and `merge` trusts forward to skip its redundant defensive recheck (gitignored, machine-local). Distinct from `.docex/runs/`: `runs/` answers "did this invocation pass"; `checks/` answers "what tree a passing check blessed."
