...
| `docs <scaffold\|check\|adr>` | both | `project.yml`, `infra.yml` (codebases), `plans/design/**` | `scaffold`: idempotent creation of the standard design-doc set under `plans/design` (never clobbers; empty standard dirs get a `.gitkeep`); `check`: read-only — exit code + per-problem report over the **missing-standard-file**, **doc-reachability** (orphan), and **ADR-index-freshness** checks. Both consult ONE canonical standard-set definition (`src/docex/docs/standard_set.py`); `check` and `adr` additionally share ONE canonical ADR-index render (`src/docex/docs/adr.py`). `docs check` also runs as a blocking gate of `docex check`; both skip when the project has no `plans/design` yet. `adr`: regenerate `adr_index.md` (all ADRs) and `adr_active.md` (accepted + not superseded) from `plans/design/adrs/` — deterministic and idempotent, each file carrying a "generated by `docex docs adr`" marker. `docs check` (and the `docex check` gate) also flag an ADR index that has drifted from its sources as a blocking problem. |
