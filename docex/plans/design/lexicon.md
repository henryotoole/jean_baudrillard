...
| Slot | A per-run-isolated env stack used to shard a suite (`--slots N`). `docex test` uses band `1..MAX_TEST_SLOTS`; `check`/`merge` run their defensive test in a reserved band above it (`check` `9..16`, `merge` `17..24`, each `MAX_TEST_SLOTS` wide) and since mod 174 may themselves shard with `--slots N`. Distinct from the doctrine `test`-env slot only in that these are docex-assigned reserved constants. |
