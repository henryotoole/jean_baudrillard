...
def test_fixed_emits_ofelia_container(tmp_path: Path):
    ...
    svc = doc["services"]["sample-dev-nightly_cleanup-nightly_cleanup-scheduler"]
...
def test_fixed_no_long_running_block_for_job(tmp_path: Path):
    ...
    assert "sample-dev-nightly_cleanup-nightly_cleanup" not in services
    assert "sample-dev-nightly_cleanup-nightly_cleanup-otelcol" not in services
    # The ofelia container IS present.
    assert "sample-dev-nightly_cleanup-nightly_cleanup-scheduler" in services
...
def test_fixed_other_services_unaffected(tmp_path: Path):
    ...
    assert "sample-dev-api-web-otelcol" in services


def test_fixed_ini_schedule_and_image(tmp_path: Path):
    doc = _dev_compose(_compile(_FIXED, tmp_path))
    ini = doc["configs"]["ofelia_nightly_cleanup-nightly_cleanup"]["content"]
    assert '[job-run "nightly_cleanup-nightly_cleanup"]' in ini
...
def test_fixed_ini_uses_bare_gcfg_lists(tmp_path: Path):
    ...
    assert "[" not in ini.replace('[job-run "nightly_cleanup-nightly_cleanup"]', "")
...
def test_fixed_ini_stage_bakes_absolute_deploy_path(tmp_path: Path):
    ...
    ini = stage["configs"]["ofelia_nightly_cleanup-nightly_cleanup"]["content"]
...
def test_test_env_omits_ofelia_fixed(tmp_path: Path):
    ...
    assert "sample-test-nightly_cleanup-nightly_cleanup-scheduler" not in doc["services"]
    assert "ofelia_nightly_cleanup-nightly_cleanup" not in (doc.get("configs") or {})
    # Suppression is scheduler-scoped: the ordinary web service stays.
    assert "sample-test-api-web" in doc["services"]
...
def test_dev_still_emits_ofelia(tmp_path: Path):
    ...
    doc = _dev_compose(_compile(_FIXED, tmp_path))
    assert "sample-dev-nightly_cleanup-nightly_cleanup-scheduler" in doc["services"]
    assert "ofelia_nightly_cleanup-nightly_cleanup" in doc["configs"]
...
def test_elastic_emits_scheduler_schedule(tmp_path: Path):
    ...
    assert 'resource "aws_scheduler_schedule" "nightly_cleanup-nightly_cleanup" {' in hcl
...
def test_elastic_emits_invocation_role(tmp_path: Path):
    hcl = _stage_hcl(_compile(_ELASTIC, tmp_path))
    assert 'resource "aws_iam_role" "nightly_cleanup-nightly_cleanup_scheduler" {' in hcl
    assert 'Service = "scheduler.amazonaws.com"' in hcl
    assert 'resource "aws_iam_role_policy" "nightly_cleanup-nightly_cleanup_scheduler" {' in hcl
    assert 'Action   = "ecs:RunTask"' in hcl
    assert 'Action   = "iam:PassRole"' in hcl
    assert "Resource = aws_ecs_task_definition.nightly_cleanup-nightly_cleanup.arn" in hcl


def test_elastic_scheduler_task_def_has_no_sidecar(tmp_path: Path):
    hcl = _stage_hcl(_compile(_ELASTIC, tmp_path))
    nc = _slice_td(hcl, "nightly_cleanup-nightly_cleanup")
    assert "otelcol" not in nc
    # No ecs_service / target_group for a scheduler.
    assert 'resource "aws_ecs_service" "nightly_cleanup-nightly_cleanup"' not in hcl
    assert 'resource "aws_lb_target_group" "nightly_cleanup-nightly_cleanup"' not in hcl
...
def test_elastic_web_service_still_has_sidecar(tmp_path: Path):
    ...
    hcl = _stage_hcl(_compile(_ELASTIC, tmp_path))
    api = _slice_td(hcl, "api-web")
    assert 'name = "api-web-otelcol"' in api
