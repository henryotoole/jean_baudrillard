...
# ---------------------------------------------------------------------------
# Mod 101 — rule 25's scheduler clause. Committed rule 25 no longer carries a
# scheduler clause, but `role: scheduler` is not retired until Mod 116, so the
# guard is kept (renamed only) rather than leaving a live role unguarded
# across three mod boundaries.
# ---------------------------------------------------------------------------
...
def test_uses_scheduler_rejected():
    src = _src(
        _api(web=_proc("web", "web", uses=["jobs.nightly"])),
        _codebase("jobs", _NIGHTLY),
    )
    hits = _hits(src, "rule_25_uses_scheduler")
    assert len(hits) == 1
...
def test_uses_non_scheduler_in_the_same_document_is_clean():
    ...
    src = _src(
        _api(web=_proc("web", "web", uses=["api.worker"])),
        _codebase("jobs", _NIGHTLY),
    )
    assert _issues(src) == []


def test_uses_scheduler_reported_once_not_alongside_an_unresolved():
    """One entry, one issue: the existence check `continue`s before the role
    check can also fire."""
    src = _src(
        _api(web=_proc("web", "web", uses=["jobs.missing"])),
        _codebase("jobs", _NIGHTLY),
    )
    hits = _hits(src, "rule_25_")
    assert [i.rule for i in hits] == ["rule_25_unresolved_uses"]
