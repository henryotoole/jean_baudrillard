...
Covers, with a fake docker (no real containers):
  ...
  * the sharded orchestrator (``_run_test_sharded`` / ``_run_one_slot``) —
    unit-once, N integration slots, per-slot injection, keep-failed-up teardown;
...
class _UnitSpy:
    """Records calls to the stubbed ``run_test_unit`` and returns a scripted rc."""

    def __init__(self, rc: int = 0):
        self.calls: list[str | None] = []
        self.rc = rc

    def __call__(self, ctx, docker, *, selector=None):
...
def sharded_env(tmp_path, monkeypatch):
    ...
    unit run). ``exec_service_key`` still derives real slotted names; with no
    compiled slot file present it skips verification and returns the derivation.
    """
    ...
    unit_spy = _UnitSpy()
    monkeypatch.setattr(test_mod, "run_test_unit", unit_spy)
    compiled_slots: list[int] = []
    monkeypatch.setattr(
        compile_mod, "compile_slot",
        lambda _ctx, _env, k: compiled_slots.append(k),
    )
    return ctx, unit_spy, compiled_slots


def _integration_env_calls(fake):
    return [
        c for c in fake.calls
        if c[0] == "compose_run_one_off_env" and c[2] == ("./test_integration.sh",)
    ]
...
def test_sharded_unit_once_three_integration_slots(sharded_env, fake_docker):
    from docex.orchestrate.test import _run_test_sharded

    ctx, unit_spy, compiled_slots = sharded_env
    rc = _run_test_sharded(
        ctx, fake_docker, tiers=("unit", "integration"), selector=None, slots=3,
    )
    assert rc == 0

    # Unit runs EXACTLY once (no-stack fail-fast gate), never sharded.
    assert unit_spy.calls == [None]
    ...
    # The integration shim carries DOCEX_TEST_SLOT=k / DOCEX_TEST_SLOTS=3 for
    # each of the three slots (and NOT a selector — none was set).
    env_calls = _integration_env_calls(fake_docker)
    ...
    # migrate.sh one-offs never carry the slot injection (integration-only).
    mig_env = [
...
def test_sharded_selector_composes_with_slot(sharded_env, fake_docker):
    ...
    ctx, _unit_spy, _slots = sharded_env
    rc = _run_test_sharded(
        ctx, fake_docker, tiers=("integration",), selector="tests/integration/foo.py",
        slots=2,
    )
    assert rc == 0
    for _tag, _svc, _cmd, items in _integration_env_calls(fake_docker):
        d = dict(items)
        assert d["DOCEX_TEST_SELECTOR"] == "tests/integration/foo.py"
        assert d["DOCEX_TEST_SLOTS"] == "2"


def test_sharded_integration_only_skips_unit(sharded_env, fake_docker):
    from docex.orchestrate.test import _run_test_sharded

    ctx, unit_spy, _slots = sharded_env
    rc = _run_test_sharded(
        ctx, fake_docker, tiers=("integration",), selector=None, slots=2,
    )
    assert rc == 0
    assert unit_spy.calls == []  # unit-once skipped when unit not in tiers.
...
def test_sharded_failed_slot_left_up_passing_torn_down(sharded_env, fake_docker):
    ...
    # Slot 2's integration shim fails (finer per-(svc,cmd) scripting).
    fake_docker.exit_codes[
        ("exit", "compose_run_one_off", slot2_key, ("./test_integration.sh",))
    ] = 5

    rc = _run_test_sharded(
        ctx, fake_docker, tiers=("integration",), selector=None, slots=3,
    )
...
def test_sharded_unit_failure_gates_fanout(sharded_env, fake_docker):
    from docex.orchestrate.test import _run_test_sharded

    ctx, unit_spy, compiled_slots = sharded_env
    unit_spy.rc = 9  # unit tier red.
    rc = _run_test_sharded(
        ctx, fake_docker, tiers=("unit", "integration"), selector=None, slots=3,
    )
    assert rc == 9
    assert unit_spy.calls == [None]
    # The expensive integration fan-out never started.
    assert compiled_slots == []
