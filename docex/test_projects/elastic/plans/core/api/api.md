...
| `api.worker` | `worker` | `entrypoints/worker.py` | `internal` | 8081 (Service-Connect-discoverable) |
...
`api.worker` is never routed — never an ALB target — and it declares **no `health_check_path`**, because rule 33 confines that field to `web`-network core services. It carries `port: 8081` because `api.web` **addresses its `rpc` surface** directly (`POST /drain`), and being directly addressed is what obliges a port under rule 32's positive arm. Not because a `uses` target must be probeable: probeability is a container concern, answered by `./health.sh worker` inside the task, and a non-`web` core service needs no HTTP surface of any kind for it (`healthchecks.md § What this doctrine does not do`). On elastic that same port is what registers the worker's Service Connect name, which is how `api.web` resolves it.

`api.clock` is never routed **and is not a `uses` target either** — nothing uses it. It carries no `port`, no `health_check_path`, and no surface, and yet a clock gets **no exemptions**: it is an ordinary long-running core service and it supplies a container probe like every other one, `./health.sh clock`, sourced from its cron loop's tick file and emitted as the task definition's container `healthCheck`. The consequence is worth stating plainly, because it is easy to misread as an oversight: **nothing external can reach the clock at all.** It binds no application socket — inside the container the only `LISTEN` entry in `/proc/net/tcp` is docker's embedded DNS resolver at `127.0.0.11`, which every container has — and having no `port` it registers nothing in Service Connect either, joining the namespace as a client-only member that resolves peers and that nothing can resolve. Nothing proxies it and the stage tester cannot see it, so the clock's liveness is enforced by the **orchestrator acting on that probe alone** — here ECS killing and replacing the task, docker merely reporting `unhealthy` on the fixed companion. That is real enforcement, but it is local (`clock.md § Caveats`).
...
**Foundation difference, and on this foundation it has teeth.** ECS **kills and replaces** a task whose essential container fails its probe, so on `stage`/`prod` the probe is a live restart trigger rather than a report — which is why the role tables also emit `startPeriod: 10` here and only here, giving a loop time to complete its first iteration before an absent tick file can kill the task. On the fixed companion docker only *reports* `unhealthy`. In this project's own `dev` and `test` environments, which compile to fixed compose stacks, the probe behaves the fixed way; the elastic behaviour is only exercised on `stage` and `prod`. Detection is not instantaneous anywhere: the tick goes stale, the next scheduled probe fails, the retry count is exhausted. That lag is deliberate and is the price of not flapping (`healthchecks.md § The orchestrator carries the result`).
...
- **`web`** curls its own `http://localhost:8080/health`. A service driven by a request cycle is nearly self-checking: if it accepts a connection and routes a trivial request, it is serving. This is the one place in the file where curling yourself is legitimate. Note that the ALB target group probes the *same* path over the network, for the different reason that a load balancer cannot run a command inside a container.
