...
`clock`) — one build artifact, one image, one ECR repo, one image tag, one
`…-migrate` task-definition family per release, started three different ways by
three entrypoints.
...
- The smoke walk no longer covers the **two-codebase** shape — notably the
  two-ECR-repo count, since one codebase compiles to exactly one ECR repo. That
  loss is deliberate; what the second codebase used to exercise, and what its
  absence costs the walk, is recorded in
