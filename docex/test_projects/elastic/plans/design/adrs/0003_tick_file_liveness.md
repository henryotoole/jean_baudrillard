...
The probe also runs as a **separate process** from the service (ECS container
health on elastic, docker's healthcheck on the fixed companion), so whatever it
reads has to be observable from outside the process.
...
**only** because a load balancer reads it and has no other channel to ask
(`healthchecks.md § web services also serve GET /health`). On elastic the ALB
target group probes that same path over the network. `api.clock` proves the
...
- On elastic the probe has **teeth**: ECS kills and replaces a task whose
  essential container fails its probe, which is why the role tables also emit
  `startPeriod: 10` (elastic-only), giving a loop time to complete its first
  iteration before an absent tick file can kill the task. On the fixed companion
  docker only *reports* `unhealthy`.
