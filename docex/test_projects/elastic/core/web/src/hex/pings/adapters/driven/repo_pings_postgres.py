...
class RepoPingsPostgres(RepoPings):
    ...
    def _connect(self) -> psycopg2.extensions.connection:
        # New connection per call — fine for a smoke-test workload.
        # WHY: connection pooling would be a real-project concern; here it would
        # add complexity without exercising any additional doctrine surface.
        return psycopg2.connect(self._dsn)

    def save(self, ping: Ping) -> None:
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(
                "INSERT INTO pings (id, payload, created_at, processed_at)"
                " VALUES (%s, %s, %s, %s)",
                (ping.id, ping.payload, ping.created_at, ping.processed_at),
            )
