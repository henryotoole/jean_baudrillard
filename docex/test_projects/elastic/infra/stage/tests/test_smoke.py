...
`test_defer_and_drain_round_trip`, which crosses Service Connect to the
...
def test_diagnostics_probe_reaches_sidecar() -> None:
    ...
    response = httpx.get(f"{STAGING_URL}/diagnostics/probe", timeout=10)
    assert response.status_code == 200, response.text
    body = response.json()
...
def test_diagnostics_events_reaches_clickhouse() -> None:
    ...
    response = httpx.get(f"{STAGING_URL}/diagnostics/events", timeout=15)
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["reachable"] == "true"
...
def test_defer_and_drain_round_trip() -> None:
    ...
    """
    deferred = httpx.post(f"{STAGING_URL}/jobs/heartbeat", timeout=10)
    assert deferred.status_code == 202, deferred.text
    assert "job_id" in deferred.json()

    drained = httpx.post(f"{STAGING_URL}/jobs/drain", timeout=15)
