...
The background processor for `docex_smoke_fixed`. Polls the `pings` table for unprocessed rows, "processes" each one (a no-op stub), and marks it processed. Internal-only; no HTTP surface.
...
- No HTTP surface. No port published. No reverse-proxy exposure (declared on `internal` only).
