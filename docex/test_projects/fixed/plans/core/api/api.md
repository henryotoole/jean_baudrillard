...
`src/root.py`, and there is exactly **one** of it — not one per process type. Splitting the root would put two copies of the driven wiring in the tree, and they would drift (`internal_dependency_rules.md § Entrypoints`, rule 3).
...
`src/entrypoints/`, one module per process type; each is what that process type's `command` invokes.

- `web.py` — calls `build_app()` and hands it to uvicorn. The runtime host belongs here, not in an adapter.
- `worker.py` — owns the three things a loop-owning process type owes and an adapter must not:
...
Two, because the codebase has two boundaries and the contract path is keyed on the process type:
...
This codebase owns the schema (`schema_owned_by: api`). See [`db_schema.md`](./db_schema.md). Migrations live at `core/api/migrations/` and are driven by `core/api/migrate.sh` (dbmate), which runs **once per codebase**, not once per process type.

Because it runs per codebase, `migrate.sh` may read **service-level `env:` only**. The six `DATABASE_*` parts are declared at the `api` service level for exactly that reason; a process-scoped var such as `WORKER_HOST` would simply be absent, silently.
...
- `/health/probe` and `/health/events` are exercise endpoints for the project-local backings — they are *not* doctrine-mandated (probe and events are backings, not core process types). They exist so the stage tests can catch Service Connect / SG / EFS-mount wiring regressions on the elastic counterpart.
