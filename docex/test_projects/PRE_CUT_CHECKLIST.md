...
The post-1.0.0 doctrine separates preinfra (machine-/account-wide), projinfra (per-project), and envinfra (per-env). Preinfra must exist before any `projinfra up` will run. Use the `preinfra-setup` skill if anything below is missing or drifted.
...
- [ ] **The tag scheme above is documented in the rule of record** — the per-resource `shape_name` / `descriptor` / `infra_tier` table in [`elastic_master_network.md`](../../doctrine/infrastructure/preinfra/elastic_master_network.md), reached through the `preinfra-setup` skill. If it has drifted, fix it as part of pre-walk setup. (Earlier revisions of this box pointed at a `docex-preinfra` skill at `~/.claude/skills/`. That skill was retired and replaced by `preinfra-setup` in advance 001, and the tag scheme was never in a skill body in any case: a thread skill is a router and carries no values, so a box asserting the values live there could not have passed even before the rename.)
...
- [ ] **B.1 Project root layout** — `project.yml`, `README.md`, `CHANGELOG.md`, `.gitignore`, `bin/docex`, `core/`, `infra/`, `plans/` all present. Per [`inception.md`](../../doctrine/practices/inception.md) PART I step 8 and [`infrastructure.md § Repository Structure`](../../doctrine/infrastructure/infrastructure.md#repository-structure).
...
- [ ] **B.12 Migrations idempotent + dbmate-formatted** — every `core/<codebase>/migrations/*.sql` is idempotent (`CREATE ... IF NOT EXISTS`) and carries both `-- migrate:up` and `-- migrate:down` sections. **The `down` half is dbmate's file format, not a path the pipeline ever takes**: [`databases.md § Migrations`](../../doctrine/practices/databases.md#migrations) requires migrations to be idempotent and **forward-only** — "the doctrine never reverses a schema, even on rollback". (This box was titled "idempotent + reversible" and cited that same section as its authority for the opposite claim. The grep passes either way, so it reported green while telling its reader something the doctrine forbids; mod 134b corrected the identical claim in the seeds' own `db_schema.md` and did not reach this file.)
...
- [ ] **B.15 Data-plane names hyphenate** — `grep -rE '{project_name_with_underscores}' infra/output/` (e.g. `docex_smoke_elastic`) finds only AWS record-key identifiers (IAM, SSM, DDB, CloudWatch log-group names), tags, comments, ECR repo names, fixed-foundation ansible host paths and vars (`deploy_root`, `project` — the underscored form there is doctrine-*mandated*, since [`configurable.md`](../../doctrine/infrastructure/configurable.md#tte-vars) specifies the fixed production TTE store at `/opt/${project}/${env}/tte.env`), and OTel resource attributes such as `service.namespace`. None of those is a data-plane name. No occurrence of an underscored project segment on a docker network/container/volume name, ECS Service Connect namespace, Route53 zone or record, or ACM cert. Per mod 030's hyphen-on-data-plane rule plus mod 046's leak fix.
