...
- [ ] **B.9 Provider contracts present** — every provider has a contract at `infra/contracts/<svc>.<proc>.<format>.yml`. The path is keyed on the **core service**, unconditionally. **The provider set is (`consumes` targets) ∪ (`web`-network core services)** — both arms load-bearing, so a non-web `worker` that anything consumes needs a contract even though nothing routes to it. The format follows from the provider's **`role`**: `role: web` → `openapi`, `role: worker` → **`asyncapi`**. `scheduler` core services are exempt (cron invokes them and nobody else does). Per [`contracts.md`](../../doctrine/infrastructure/contracts.md).
