...
The `schema_owned_by:` field on database roles (e.g. `relational_db`) names the core service that owns and migrates the schema; this is doctrine-enforced as a 1:1 invariant to prevent two services racing on the same database.
