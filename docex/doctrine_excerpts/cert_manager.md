# cert_manager

TLS certificate provisioning and rotation.

- **Fixed:** built into the project's own traefik (one per project, at the project tier — see `why reverse_proxy`), so each project owns its certs. Traefik talks Let's Encrypt ACME, fetches certs lazily as new subdomains appear in its router config, and renews them on its own schedule. Zero project-side configuration.
- **Elastic:** AWS ACM, a project-tier resource. One wildcard cert (`*.<domain>`) covers every env's ALB listener. ACM handles renewal automatically.

Both paths satisfy the doctrinal guarantee that no project ever embeds a private key in its repo or worries about cert expiry as an operational concern.

Doctrine reference: `infrastructure/shape.md`.
