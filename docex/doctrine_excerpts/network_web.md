...
- **Fixed:** the container gets Traefik discovery labels (`traefik.enable=true`, host rules, TLS entry-point) and joins the `{project}_{env}_web` docker network, which the machine-wide Traefik watches. Traefik routes requests for the env's subdomain to the container.
