...
- **Fixed:** a plain user-defined docker bridge with no published host ports, so nothing outside the host can reach it. Services reach each other by container name (which equals `${global_service_name}`). Docker's `internal: true` flag is deliberately **not** used — it adds no ingress protection over this shape, and it would strip the bridge's masquerade rule and kill egress.
