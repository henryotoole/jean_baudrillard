...
6. **Review**: You review the implementation:
	...
	4. **Close the test step on a full run.** You may iterate during the cycle with scoped runs (`docex test [subset]`) to keep the loop fast, but the cycle **closes** on a full run of the suite, all green. Choose the scope by your own judgment as the author of the diff — nothing computes an "affected" set from the changes.
