...
**Seeing telemetry in dev:** in `dev`/`test` the sidecar's exporter is `debug`, which dumps every signal to the *sidecar's* stdout — read it with `docker compose logs -f <svc>-otelcol`. That is the dev "watch the telemetry" path; no application-side echo is needed.
