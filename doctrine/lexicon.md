...
| Environment | "env" | A copy of all environment-tier infrastructure that serves a distinct purpose: `dev`, `test`, `stage`, and `prod`. The env is a singleton by default; a **fixed** env may be instantiated into multiple isolated **slots** on one machine, which share the env's identity and configurable values and differ only by a slot segment woven into every physical resource name. See [infrastructure.md § The slot axis](./infrastructure/infrastructure.md#the-slot-axis). |
