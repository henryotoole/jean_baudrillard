...
2. Listener certs - These cap out too, somewhat more harshly. They also limit us.
...
Decentralized egress is simply too costly. It involves giving each project its own NAT, which consumes an EIP (a finite resource) and costs about $400 / yr.

### Decentralized Project Networks

AWS VPCs aren't costly, but connecting them to a Transit Gateway (and then to a NAT) is almost as expensive as a NAT itself. The solution is one big "master network"; a main VPC with a NAT and all projects in its scope.
