...
This is why the doctrine's resilience requirement does not cover it. [`uses`](../cicl.md#uses-relationships) obliges every core service to tolerate a dependency being momentarily absent — timeouts, backoff, clean failures. That answers **reachability**: a target that is down, restarting, or briefly unroutable will come back, and a client that keeps trying will find it. **Resolvability** is a different failure with the same appearance. The name does not exist for the calling task and will not begin to exist while that deployment lives, so there is nothing for backoff to converge on. No discipline on the consumer's side escapes it, which is why the fix has to live in the release rather than in the application.
