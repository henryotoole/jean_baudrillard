...
- The environment's `secrets` (per [`secrets.md`](./secrets.md))
...
1. **Push secrets to SSM Parameter Store** per [`secrets.md`](./secrets.md). If this fails, no further steps run — the release fails cleanly with no infrastructure changes attempted.
