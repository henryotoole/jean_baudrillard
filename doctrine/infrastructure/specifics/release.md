...
- **Ties break toward redeploying.** Both timestamps are AWS-server-issued, but they come from two different services, so sub-second skew is possible: a task that started at exactly a name's registration time counts as too old. A false positive costs one rolling deploy; a false negative costs a permanently broken env that exits 0. Never round toward silence — and never widen the tie into a grace window, which would fire on nearly every consumer on every first release, since the name is created with the ECS service before any of its tasks exist.
...
**Fixed foundations need none of this.** Docker network DNS resolves a sibling container whenever it exists, with no launch-time name freeze.
