...
The bootstrap creates the state backend directly (via the AWS API, since OpenTofu itself can't yet run), and then applies the project-tier HCL emitted by [`docex compile`](../docex.md#compile) at `infra/output/project/main.tf`. Once both are in place, each env's `release` is just an env-tier `tofu apply` against a backend that already has every cross-environment dependency available.
...
- **ECR repositories** — one per core service (`<project>/<svc>`), with `MUTABLE` tag-immutability. Created here rather than ad-hoc by `docex containerize` so the registry surface is part of project state.
...
`docex bootstrap` is the convenience command that creates the resources described above. It is **idempotent** at every step:
...
ACM DNS validation requires the project's Route53 zone to be reachable via the public DNS chain — which it only is once the operator has NS-delegated from the parent (registrar or parent hosted zone). To avoid hanging the apply on a delegation the operator has yet to perform, `docex bootstrap` splits the project-tier apply into two phases:
...
2. **Phase 2 — full apply.** Once the operator has NS-delegated the project domain (records take a few minutes to propagate), they re-run `docex bootstrap`. This time the zone is already in state, so the bootstrap runs an untargeted `tofu apply`. The ACM certificate validates against the now-reachable zone, the VPC and subnets come up, and the ECR repositories appear. If the delegation hasn't propagated, ACM cert validation fails the apply — the bootstrap surfaces a hint pointing at the delegation requirement.

Phase detection is observed from `tofu state list`, not stored separately. The operator's mental model is single-command: `docex bootstrap`, do the delegation work it asks for, `docex bootstrap` again, done.

## When to Run It

For an elastic-foundation project, after `docex compile` and before the first `docex release` for `stage` or `prod`. Concretely:

1. Author `project.yml` and `infra/infra.yml` with `foundation: elastic`.
2. `docex compile` — emits the project-tier `main.tf` along with the env-tier ones.
3. `docex bootstrap` — creates the state backend and runs phase 1 of the project-tier apply.
4. NS-delegate the project domain to the printed name servers at the parent.
5. `docex bootstrap` — runs phase 2; project-tier infrastructure complete.

After this, releases (`docex release stage` / `docex release prod`) only need to apply env-tier HCL.
...
- Env-tier resources (ALB, RDS, ECS services, etc.) — those are emitted by `compile` into each env's `main.tf` and applied by `docex release`.
