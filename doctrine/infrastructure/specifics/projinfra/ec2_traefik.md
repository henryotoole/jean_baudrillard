...
**Labels.** `./bin/docex compile` emits the traefik labels onto each `web`-network service's container definition in the env-tier HCL. `exposedByDefault` is `false`, so only containers carrying `traefik.enable=true` are routed — and the compiler emits that label only for `web`-network process types, so a `worker` or `scheduler` process type and every backing service are never exposed regardless of whether they declare a `port`. Router names encode the env (`<service>-<process>-<env>`) so the single instance's `stage` and `prod` views don't collide. Each `web`-network process type in stage and prod gets labels of roughly the form (this example assumes `api.web` is the project's `domain_default_process` per [cicl.md § Domain](../../cicl.md#domain), which is why prod's rule carries the bare-project and bare-env alternates and stage's rule carries the bare-stage-env alternate; process types that are not the default get only the full `<service>-<process>.<env>.<project>.<apex>` host):
