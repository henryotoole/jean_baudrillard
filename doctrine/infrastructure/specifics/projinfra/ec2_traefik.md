...
**Both clusters must always exist.** traefik's ECS provider treats a `ListTasks` error on *any* configured cluster as fatal for the entire refresh — if one listed cluster is absent it builds **zero** routes, even for clusters that do exist. Because the instance's cluster list is static (baked into user_data at projinfra time, before any env release), both the `stage` and `prod` ECS clusters must exist independently of which env has been released. The doctrine guarantees this by provisioning both ECS clusters at the **project tier** (empty clusters are free); env-tier release only attaches services into them. See [shape.md § ecs_cluster](../../shape.md#elastic-foundation).
...
**Labels.** `./bin/docex compile` emits the traefik labels onto each `web`-network service's container definition in the env-tier HCL. `exposedByDefault` is `false`, so only containers carrying `traefik.enable=true` are routed — and the compiler emits that label only for `web`-network core services, so a `worker` or `clock` core service and every backing service are never exposed regardless of whether they declare a `port`. Router names encode the env (`<codebase>-<service>-<env>`) so the single instance's `stage` and `prod` views don't collide. Each `web`-network core service in stage and prod gets labels of roughly the form (this example assumes `api.web` is the project's `domain_default_service` per [cicl.md § Domain](../../cicl.md#domain), which is why prod's rule carries the bare-project and bare-env alternates and stage's rule carries the bare-stage-env alternate; core services that are not the default get only the full `<codebase>-<service>.<env>.<project>.<apex>` host):
...
- The two empty ECS clusters (`<project>-stage`, `<project>-prod`) the provider's static config lists — project-tier for all elastic projects (both reverse-proxy paths), so the provider's cluster list always resolves (see [Routing Discovery](#routing-discovery))
