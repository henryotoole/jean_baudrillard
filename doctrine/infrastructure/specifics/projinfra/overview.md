...
| Aspect | Detail |
| ------ | ------ |
| What gets created | OpenTofu state backend (S3 + DynamoDB); Route53 hosted zone; two ACM certs; the project reverse proxy (ALB *or* EC2-traefik depending on `reverse_proxy:`); one ECR repository per core service; the task-execution IAM role |
