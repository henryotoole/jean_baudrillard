...
- **A clock is invisible to staging tests.** It is not on `web`, so the stage tester cannot reach it by any route. This is no longer a special case: [staging tests](../tests.md#staging-tests) assert nothing about any core service's liveness, and every core service's health — a clock's included — is read from the orchestrator by `./bin/docex stagetest` before the tester runs. A wedged clock fails its container probe. On `elastic` the ECS scheduler replaces the task; on `fixed` Docker reports the status and restarts nothing of its own accord ([rule 33](../cicl.md#validation-rules)). Either way `./bin/docex stagetest` reads that status and fails the release if it stays down.
