...
33. Every `web`-network core service declares `health_check_path`, and no core service off the `web` network declares one. The field is what an ALB probes. It is required regardless of which reverse proxy a project runs, because a project must stay portable between them — so on two of the three configurations it is a declaration nothing reads, and that is accepted rather than hidden. It is consumed by the ALB target group on `elastic` **with the default `reverse_proxy: alb`** — and there alone. On the `ec2_traefik_eip` / `_pip` variants the compiler still emits a target group carrying the path, but no ALB attaches to it, so the value is inert; traefik's ECS provider routes on `lastStatus == RUNNING`, which is a lifecycle state and not a health verdict. On `fixed` it has **no consumer at all** — the compiler emits no health-aware load-balancer labels, so the project traefik performs no probing of its own — and the declaration is required there anyway so a project stays portable between foundations. Whether traefik's Docker provider *passively* withholds routing from a container Docker has marked unhealthy is a property of that tool which nothing in this doctrine verifies: **do not rely on it.** On `fixed` the container probe has exactly two consumers, and neither reroutes traffic — Docker, which reports a status and restarts nothing of its own accord, and [`docex stagetest`](./cicd.md#staging-tests), which reads that status and fails a release on it.