...
3. Perform documentation checks against `plans/design` (design-doc validation; the same checks as [`docex docs check`](./docex.md#docs)). All three are **blocking**.
...
1. Verify preconditions. If any fail, abort before touching the environment:
	...
	4. `<target_version>`'s `infra.yml` declares a `cicl_version` this `docex` compiles. Rollback recompiles the target's `infra.yml` with the *current* compiler (step 3), so a target declaring a `cicl_version` this compiler no longer accepts cannot be rebuilt; the check reads the tag's `infra.yml` directly and aborts with a fix-forward message. See [cicl.md § CICL Version](./cicl.md#cicl-version).
