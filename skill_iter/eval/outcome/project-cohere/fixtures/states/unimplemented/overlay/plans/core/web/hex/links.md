...
- **ContLinks** — the module's use cases:
  ...
  - `shorten(target_url, alias)` — custom aliases: when the caller supplies an
    `alias`, that alias is used as the short code directly instead of a
    generated one, letting users pick memorable links. The alias is rejected if
    it is already taken.
