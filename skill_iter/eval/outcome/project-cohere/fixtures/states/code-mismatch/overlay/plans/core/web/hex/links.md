...
- **ShortLink** — a value object binding a `code` to a `target_url`. Its
  invariants: the code is exactly 6 characters, is alphanumeric, and the
  target URL must be an http(s) URL.

## Driving Ports

- **ContLinks** — the module's use cases:
  - `shorten(target_url)` — generate a fresh 6-character alphanumeric code for
    the URL, persist the mapping, and return the code.
  - `resolve(code)` — return the target URL for a code, or raise
    `LinkNotFoundError` when the code is unknown.

## Driven Ports

- **RepoShortLink** — persistence for short links (`save`,
  `get_by_code`). `get_by_code` raises when a code is missing.
