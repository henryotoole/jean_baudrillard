...
- Per core service on the `web` network:
  `https://<codebase>-<service>.dev.<project>.<apex_domain>`
  (the codebase and core service segments are joined by a hyphen and occupy
  one label; the project segment is hyphenated too — `my_project` → `my-project`).
