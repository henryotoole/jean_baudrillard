...
2. Check for skill coverage of conditional strata.
3. Check for detail creep into resident strata.
...
Run the executor first: `python3 executor/linkcheck.py` (from this skill's directory). It deterministically reports checks 1 (broken links / bad heading anchors) and 3 (duplicate filenames) across the whole corpus, and exits non-zero if anything is found. Do not hand-derive GitHub anchor slugs — the executor already encodes them correctly. Then dispatch a subagent for check 2 (spelling and grammar), which is not deterministic.
