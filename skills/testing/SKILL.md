...
- **Scope, since codebase and core service split:** a flow test is scoped to the **codebase** (one composition root, so a scenario entering at `web` can reach `worker`'s handler in-process); a contract test asserts about one **core service's** boundary. Both nonetheless *run* in the codebase's exec-service one-off container, under the single `test.sh`.
