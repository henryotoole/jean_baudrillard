...
Practices are discussed [here](TODO). The rest of this document handles the infrastructure side, but will occasionally touch on practices.
...
1. **Application-Level** signals originate in application code. This could be a handled exception or a log line.
2. **Infrastructure-Level** signals originate within an infrastructure component. This might be an ECS-reported `RunningTaskCount` metric or load balancer access logs.
...
So we have three kinds of telemetry signals and two places from which they can originate. They also have two different "flows" throughout project infrastructural components split by origin (with just a little overlap):
1. **Application Telemetry Flow** - Rich, structured, and queryable telemetry data that primarily originates at the application level. Exclusively in OTel. Leavened with *some* infrastructure-level signals. Helps us answer "*what's actually happening and why?*".
2. **Infrastructure Telemetry Flow** - Fast, simple, and automated. Catches service status and health and is used to trigger automatic responses like launching new containers or load balance switching. Uses no application-level signals.

The infrastructure telemetry flow's specifics will be driven by foundation / provider requirements. It will therefore primarily be a deterministic `docex` concern and consist mostly of compiling the output of `infra.yml` correctly. It is mentioned here to illustrate that there *will be* telemetry outside of the below [application telemetry flow](#application-telemetry-flow); we just won't speak much more about it here.

### Application Telemetry Flow

Application-level telemetry signals flow like this:
*Core Services -> Collector Sidecars -> Observability Backend*
and a handful of included infrastructure-level-origin signals flow like this:
*Infrastructure Resource -> Observability Backend*
...
Each collector forwards signals to the project-wide observability backend. This backend will always be [prerequisite infrastructure](TODO), both on `fixed` and `elastic` foundations. It will be centrally configured for each project with the `observability_backend_url` field and all sidecars will forward application telemetry to that URL.
...
Telemetry signals need authentication for transmission between the collector sidecar and the observability. The standard way is to give the sidecars an API key which will be included as an HTTP header in signal requests. This API key will be delivered to the sidecars with the doctrine's standard env var / secret delivery mechanisms. It will be set in the relevant `secrets/<env>.env` file, always as `TELEMETRY_API_KEY`.

It unfortunately falls to the developer to log into the relevant HyperDX instance for the project, create or select the relevant team, and retrieve the team's ingestion API key. It's acceptable to share API keys across 

#### From Infrastructure Resources

There are a couple specific infrastructure-level-origin signals which are included in the application telemetry flow:
**Service Level Health Check Metrics** - Lets us know if health checks failing are associated with a problem.
**Container Resource Metrics** - CPU/memory metrics which allow correlation of application performance with hardware limitations.
